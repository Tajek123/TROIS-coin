...
{
    // Windows: C:\Users\Username\AppData\Roaming\TROIS
    // macOS: ~/Library/Application Support/TROIS
    // Unix-like: ~/.bitcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TROIS";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/TROIS";
#else
    // Unix-like
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure TROIS Core copyright is not removed by accident
    if (copyright_devs.find("TROIS Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The TROIS Core developers";
    }
