...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a TROIS address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new TROISAddressEntryValidator(parent));
    widget->setCheckValidator(new TROISAddressCheckValidator(parent));
}
...
bool parseTROISURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!TROISUnits::parse(TROISUnits::ISC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseTROISURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseTROISURI(uriInstance, out);
}

QString formatTROISURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(TROISUnits::format(TROISUnits::ISC, info.amount, false, TROISUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "TROIS.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "TROIS (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("TROIS (%s).lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=TROIS\n";
        else
            optionFile << strprintf("Name=TROIS (%s)\n", chain);
